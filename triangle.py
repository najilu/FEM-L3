...
class Triangle:
    ...
    def __init__(self, points: list[Point], id: int, physical_tag: int = -1):
        ...
        (Le paramètre est ici uniquement présent pour la modularité du code, si l'on voulet implémenter les
        éléments finis en 3 dimensions.)
        """
        self.points = points
        self.id = id
    ...
    def area(self) -> float:
        """
        Calcul de l'aire du triangle à l'aide de la formule d'héron
        :return: floating
        """
    ...
    def gaussPoint(self, order:int=2) -> float:
        """
        Calcul des informations nécéssaire à la quadrature de Gauss
