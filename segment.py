...
class Segment:
    ...
    def area(self) -> np.floating:
        """
        Calcul l'aire du rectangle (c'est-à-dire sa longueur)
        :return: float
        """
    ...
    def jac(self) -> np.floating:
        """
        Calcul le jacobien du changement de variable qui envoie le segment de référence dans un segment particulier
        (Ne sert pas ici car on considère un maillage 2d mais la fonction est là pour le cas ou l'on vodurait implémenter
    ...
    def get_boundary(self, i:int):
        """
        Retourne le premier ou le deuxième sommet, ils sont déjà orienter dans le sens trigo grâce à gmsh.
